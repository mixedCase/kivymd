...
from theme import ThemeBehavior

class MaterialLabel(ThemeBehavior, Label, BackgroundColorCapableWidget):
	""":class:`MaterialLabel(**kwargs)` uses by default the Roboto font. With
	:attr:`font_style` and :attr:`theme_style` you can choose from some
	pre-defined styles that's described in the Material Design guide lines.
	...
	font_style = OptionProperty(
		'Body1',
		options=['Body1', 'Body2', 'Caption', 'Subhead', 'Title', 'Headline',
		         'Display1', 'Display2', 'Display3', 'Display4', 'Button',
		         'Icon'])
	"""With :attr:`font_style` you can choose from a list of pre-defined font
	styles:
	...
	Available styles:
		...
		* 'Icon'		- Material Design Iconic Font,	24dp
	...
	theme_text_color = OptionProperty(
		'Primary', options=['Primary', 'Secondary', 'Hint', 'Error'])
	"""With :attr:`theme_text_color` you can choose from a list of
	pre-defined text colors. Look at the documentation on
	:class:`~kivymd.theme.ThemeManager` for more information.
	...
	theme_style = OptionProperty(
		None, options=['Light', 'Dark', 'Custom'], allownone=True)
	"""Use :attr:`theme_style` to override the default
	:class:`~kivymd.theme.ThemeManager.theme_style`.
	...
	"""Use :attr:`text_color` in combination with :attr:`theme_style` set to
	'Custom' to give the text a custom color.

	You can pass the color as rgba or as a theme palette tuple,
	eg ['Green', '500']
	...
	"""If :class:`MaterialLabel.font_style` is set to 'Icon', use
	:attr:`icon` to set the icon. For a list of available icons, go to::
	...
	def __init__(self, **kwargs):
		...
		self.bind(theme_text_color=self._update_color,
		          ...
		          size=self._fix_size)
		self.disabled_color = \
			App.get_running_app().theme_cls.disabled_text_color()
	...
	def on_font_style(self, instance, style):
		...
		if self.font_style == 'Icon':
			self.text = u"{}".format(m_res.get_icon_char(self.icon))

		self.font_name = self._font_styles[self.font_style][0]
		self.bold = self._font_styles[self.font_style][1]
		if m_res.DEVICE_TYPE == 'desktop' and \
				self._font_styles[self.font_style][3]:
			new_font_size_modulus = self._font_styles[self.font_style][3]
		else:
			new_font_size_modulus = self._font_styles[self.font_style][2]
		if style == 'Icon':
			self.font_size = dp(new_font_size_modulus)
		else:
			self.font_size = sp(new_font_size_modulus)
	...
	def _update_color(self, *args):
		...
		if style == 'Custom':
			if len(self.text_color) == 2:
				self.color = m_res.get_rgba_color(self.text_color)
				return
	...
	def _get_color(self, style):
		if self.theme_text_color == 'Primary':
			return \
				App.get_running_app().theme_cls.primary_text_color(style=style)
		elif self.theme_text_color == 'Secondary':
			return \
				App.get_running_app().theme_cls.secondary_text_color(style=style)
		elif self.theme_text_color == 'Hint':
			return \
				App.get_running_app().theme_cls.hint_text_color(style=style)
		elif self.theme_text_color == 'Error':
			return \
				App.get_running_app().theme_cls.error_color
